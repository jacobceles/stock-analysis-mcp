...
async def generate_plot_data_agent(
    data: str,
    ...
    generate_plot_code_output = await agent_tool.run_async(
        args={"request": data}, tool_context=tool_context
    )
...
async def generate_plot(tool_context: ToolContext) -> bool:
    ...
    try:
        ...
        print(f"Successfully saved Python artifact '{plot_filename}' as version {version}.")
        return True
...
def sample_equally_spaced(x, y, max_points=10) -> tuple:
    n = len(x)
